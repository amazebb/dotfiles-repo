...
setup_prompt() {
    ...
    prompt_segment_tail() {
        ...
        if [[ -n ${vcs_stline} ]]; then
            local branch="${vcs_stline%% *}"
            local modifiers="${vcs_stline#$branch}"
            if [[ ${modifiers} =~ "[+~?]" ]]; then
                b='%{%K{yellow}%}'
                f='%{%F{yellow}%}'
            else
            ...
            echo -n "$b$s $g ${vcs_stline}$f $n$ra%f%k"
        else
            echo -n "$n$s%f%k"
        fi
    }
